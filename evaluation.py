...
eval_dataset = EvalDataset(data_dir=eval_data_dir)
...
def save_overlay_image(image_path, mask_path, prediction, overlay_path):
    ...
    result_image = Image.new("RGB", (input_image.width * 2, input_image.height))
    result_image.paste(input_image, (0, 0))
    result_image.paste(ground_truth_mask, (input_image.width, 0))
    result_image.paste(predicted_mask.convert("RGB"), (input_image.width * 2 // 3, 0))
    result_image.paste(overlay, (input_image.width * 2 // 3, 0))
